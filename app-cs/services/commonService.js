angular.module('business-insights').factory('commonService', ['$q','$http', '$localStorage', function($q, $http, $localStorage){
   	...
   	function alertMessage (message, type, $alert){
   		    console.log('alert',message, type, $alert);
			...
			return function(){$alert({title: type, 
				content: (type=='error')? ((message)? (alertMessage + hint + message):alertMessage):message,
				 placement: 'top-right', type: type, animation: animation, keyboard: true, duration:3});}();
		} 
